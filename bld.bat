...
REM else ifeq ($(CONFIG),conda-windows)
REM CXX = x86_64-w64-mingw32-gcc
REM LD = x86_64-conda_cos6-linux-gnu-gcc
REM LDFLAGS := $(filter-out -rdynamic,$(LDFLAGS)) -static
REM LDLIBS := $(filter-out -lrt,$(LDLIBS))
REM CXXFLAGS := $(filter-out -fpermissive,$(CXXFLAGS))
REM CXXFLAGS += -std=c++11 -Os
REM ABC_ARCHFLAGS="-DSIZEOF_VOID_P=8 -DSIZEOF_LONG=8 -DSIZEOF_INT=4 -DWIN32_NO_DLL -DHAVE_STRUCT_TIMESPEC -D_POSIX_SOURCE -fpermissive -w"
REM ABCMKARGS="CC=\"x86_64-w64-mingw32-gcc\" CXX=\"x86_64-w64-mingw32-gcc\" OPTFLAGS=\"-O\" \
REM 				ARCHFLAGS=\"$(ABC_ARCHFLAGS)\" ABC_USE_NO_READLINE=1"
